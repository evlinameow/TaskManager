...
namespace TaskManager
{
    public partial class Form1 : Form
    {
        ...
        private void KillName(Process process)
        {
            process.Kill();
            process.WaitForExit();
        }
        private void KillID(int pid)
        {
            if (pid == 0)
            {
                return;
            ...
            {
                KillID(Convert.ToInt32(item["ProcessID"]));
            }
            try
            {
        ...
        private void toolStripButton3_Click(object sender, EventArgs e)
        {
            try
            {
                if (listView1.SelectedItems[0] != null)
                {
                    Process processToKill = processes.Where((x) => x.ProcessName ==
                    listView1.SelectedItems[0].SubItems[0].Text).ToList()[0];
                    KillID(GetParentProcessId(processToKill));
                    GetProcesses();
                    RefreshProcessesList();
                }
            }
        ...
        private void завершитьПроцессыToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                if (listView1.SelectedItems[0] != null)
